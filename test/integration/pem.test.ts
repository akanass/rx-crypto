...
describe('- Integration pem.service.test.ts file', () => {
  ...
  test('- `PEM.readPkcs12()` Observable must return an error if no pkcs12 is provided', (done) => {
    pem.readPkcs12('/i/do/not/exist.p12').subscribe({
      error: err => {
        expect(err.message).toBe('Cannot read properties of undefined (reading \'p12Password\')');
        done();
      }
    });
  });
